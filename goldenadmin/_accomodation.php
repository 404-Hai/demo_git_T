...
$id=$_REQUEST["id"];
$act=$_REQUEST["act"];
$sqlMenu="SELECT * FROM tbl_Menu where Menu_Module='accomodation' and lang=$lang";
  ...
  <tr>
    <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		if ($id_current!=0)
		{
			$sqlsub="SELECT * FROM tbl_Accomodation where Accomodation_ID=".$id_current." and lang=$lang";
			$rsub = mysql_query($sqlsub,$cnn);
			if ($rowsubMenu = mysql_fetch_array($rsub))
			{
				echo "<img src='skin/navpil_home.gif'>";
				echo "<a href='?mod=".$mod."&act=add&id_current=".$id_current."&kind=".$kind."&lang=".$lang."'>".$rowsubMenu["Accomodation_Name"]."</a>";
			}
		}
		?>
		</td>
...
<?php 
$sql="SELECT * FROM tbl_Accomodation where lang=$lang and Accomodation_Parent=".$id_current." order by Accomodation_ID DESC";
//================================================================================
...
?>
  	...
  	<tr style="padding:2px; font-size:11px ">
		...
		<td align="center">
		<?
		$SQL="SELECT * FROM tbl_".$mod." WHERE Accomodation_Parent=".$row["Accomodation_ID"];
		$r = mysql_query($SQL,$cnn);
		if ($rows = mysql_fetch_array($r))
		{
			echo '<a href=?mod='.$mod.'&id_current='.$row["Accomodation_ID"].'>';
			echo 'Sub Item';
			echo '</a>';
		}
		else
		...
		<td align="center">
		<input name="" type="checkbox" <? if ($row["Accomodation_Istop"]==1){?> checked <? }?> onClick="document.location.href='module/Settop.php?id=<?=$row["Accomodation_ID"];?>&mod=<?=$mod;?>&id_current=<?=$id_current;?>&kind=<?=$kind;?>';"></td>			
    	<td align="center">
		<a href="?mod=<?=$mod?>&id=<?=$row["Accomodation_ID"];?>&act=edit&id_current=<?=$id_current?>&curPg=<?=$curPg?>&lang=<?=$lang?>"><img alt="Sửa" src="skin/icon_edit.gif" width="16" height="16" border="0"></a></td>
    	<td align="center">
		<a href="module/Del.php?mod=<?=$mod?>&id=<?=$row["Accomodation_ID"];?>&id_current=<?=$id_current?>&curPg=<?=$curPg?>&lang=<?=$lang?>" onclick="javascript:return confirm('Bạn có muốn xóa không ?')">
...
<!--------------------------------------------------------------------->
	...
	if ($act=="edit")
	{
		$sqlEdit = "select * from tbl_Accomodation where Accomodation_ID=".$id;
		$result = mysql_query($sqlEdit,$cnn);	
		if ($rowEdit=mysql_fetch_array($result))
		{
			$Accomodation_Name		=$rowEdit['Accomodation_Name'];			
			$Accomodation_Image		=$rowEdit['Accomodation_Image'];
			$Accomodation_Teaser	=$rowEdit['Accomodation_Teaser'];
			$Accomodation_Content	=$rowEdit['Accomodation_Content'];
			$f_action		='Edit.php';
			$btnSubmit		="Edit";
		}
	}
	elseif (($act=="add") || ($act==""))
		{				
			$Accomodation_Name		="title";				
			$Accomodation_Image		="";				
			$Accomodation_Teaser	="qoute";
			$Accomodation_Content	="detail";
			$f_action		='Add.php';
			$btnSubmit		="Add";
		}
	?>
	...
	<tr>
   		<td><input name="Accomodation_Name" type="text" value="<?=$Accomodation_Name;?>" style="width:100%; border:1px solid #CCCCCC "></td>
  	</tr>	
	<tr>
   		<td>Image <input type="text" id="inpImgURL" name="inpImgURL" class="inpTxt" value="<?=$Accomodation_Image?>" style="width:88%; border:1px solid #CCCCCC; height:18px; ">				
	...
	<tr>
    	<td><textarea name="Accomodation_Teaser"><?=$Accomodation_Teaser;?></textarea>		
		<script>
			...
			oEdit1.REPLACE("Accomodation_Teaser");
		</script>	</td>
  	</tr>  	
  	<tr>
    	<td><textarea name="Accomodation_Content"><?=$Accomodation_Content;?></textarea>		
		<script>
			...
			oEdit2.REPLACE("Accomodation_Content");
		</script>	</td>
  	</tr>  
	<tr>
   	 	<td align="center">
