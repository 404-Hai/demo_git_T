...
$id=$_REQUEST["id"];
$act=$_REQUEST["act"];
$sqlMenu="SELECT * FROM tbl_Menu where Menu_Module='Promotion' and lang=$lang";
  ...
  <tr>
    <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		if ($id_current!=0)
		{
			$sqlsub="SELECT * FROM tbl_Promotion where Promotion_ID=".$id_current." and lang=$lang";
			$rsub = mysql_query($sqlsub,$cnn);
			if ($rowsubMenu = mysql_fetch_array($rsub))
			{
				echo "<img src='skin/navpil_home.gif'>";
				echo "<a href='?mod=".$mod."&act=add&id_current=".$id_current."&kind=".$kind."&lang=".$lang."'>".$rowsubMenu["Promotion_Name"]."</a>";
			}
		}
		?>
		</td>
...
<?php 
$sql="SELECT * FROM tbl_Promotion where lang=$lang and Promotion_Parent=".$id_current." order by Promotion_ID DESC";
//================================================================================
...
?>
  	...
  	<tr style="padding:2px; font-size:11px ">
		...
		<td align="center">
		<?
		$SQL="SELECT * FROM tbl_".$mod." WHERE Promotion_Parent=".$row["Promotion_ID"];
		$r = mysql_query($SQL,$cnn);
		if ($rows = mysql_fetch_array($r))
		{
			echo '<a href=?mod='.$mod.'&id_current='.$row["Promotion_ID"].'>';
			echo 'Sub Item';
			echo '</a>';
		}
		else
		...
		<td align="center">
		<input name="" type="checkbox" <? if ($row["Promotion_Istop"]==1){?> checked <? }?> onClick="document.location.href='module/Settop.php?id=<?=$row["Promotion_ID"];?>&mod=<?=$mod;?>&id_current=<?=$id_current;?>&kind=<?=$kind;?>';"></td>			
    	<td align="center">
		<a href="?mod=<?=$mod?>&id=<?=$row["Promotion_ID"];?>&act=edit&id_current=<?=$id_current?>&curPg=<?=$curPg?>&lang=<?=$lang?>"><img alt="Sửa" src="skin/icon_edit.gif" width="16" height="16" border="0"></a></td>
    	<td align="center">
		<a href="module/Del.php?mod=<?=$mod?>&id=<?=$row["Promotion_ID"];?>&id_current=<?=$id_current?>&curPg=<?=$curPg?>&lang=<?=$lang?>" onclick="javascript:return confirm('Bạn có muốn xóa không ?')">
...
<!--------------------------------------------------------------------->
	...
	if ($act=="edit")
	{
		$sqlEdit = "select * from tbl_Promotion where Promotion_ID=".$id;
		$result = mysql_query($sqlEdit,$cnn);	
		if ($rowEdit=mysql_fetch_array($result))
		{
			$Promotion_Name		=$rowEdit['Promotion_Name'];			
			$Promotion_Image		=$rowEdit['Promotion_Image'];
			$Promotion_Teaser		=$rowEdit['Promotion_Teaser'];
			$Promotion_Content	=$rowEdit['Promotion_Content'];
			$f_action		='Edit.php';
			$btnSubmit		="Edit";
		}
	}
	elseif (($act=="add") || ($act==""))
		{				
			...
			$Promotion_Image		="";				
			$Promotion_Teaser	="qoute";
			$Promotion_Content	="detail";
			$f_action		='Add.php';
			$btnSubmit		="Add";
		}
	?>
	...
	<tr>
   		<td><input name="Promotion_Name" type="text" value="<?=$Promotion_Name;?>" style="width:100%; border:1px solid #CCCCCC "></td>
  	</tr>	
	<tr>
   		<td>Image <input type="text" id="inpImgURL" name="inpImgURL" class="inpTxt" value="<?=$Promotion_Image?>" style="width:88%; border:1px solid #CCCCCC; height:18px; ">				
	...
	<tr>
    	<td><textarea name="Promotion_Teaser"><?=$Promotion_Teaser;?></textarea>		
		<script>
			...
			oEdit1.REPLACE("Promotion_Teaser");
		</script>	</td>
  	</tr>  	
  	<tr>
    	<td><textarea name="Promotion_Content"><?=$Promotion_Content;?></textarea>		
		<script>
			...
			oEdit2.REPLACE("Promotion_Content");
		</script>	</td>
  	</tr>  
	<tr>
   	 	<td align="center">
