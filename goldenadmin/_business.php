...
$id=$_REQUEST["id"];
$act=$_REQUEST["act"];
$sqlMenu="SELECT * FROM tbl_Menu where Menu_Module='Business' and lang=$lang";
  ...
  <tr>
    <td><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
		...
		if ($id_current!=0)
		{
			$sqlsub="SELECT * FROM tbl_Business where Business_ID=".$id_current." and lang=$lang";
			$rsub = mysql_query($sqlsub,$cnn);
			if ($rowsubMenu = mysql_fetch_array($rsub))
			{
				echo "<img src='skin/navpil_home.gif'>";
				echo "<a href='?mod=".$mod."&act=add&id_current=".$id_current."&kind=".$kind."&lang=".$lang."'>".$rowsubMenu["Business_Name"]."</a>";
			}
		}
		?>
		</td>
...
<?php 
$sql="SELECT * FROM tbl_Business where lang=$lang and Business_Parent=".$id_current." order by Business_ID DESC";
//================================================================================
...
?>
  	...
  	<tr style="padding:2px; font-size:11px ">
		...
		<td align="center">
		<?
		$SQL="SELECT * FROM tbl_".$mod." WHERE Business_Parent=".$row["Business_ID"];
		$r = mysql_query($SQL,$cnn);
		if ($rows = mysql_fetch_array($r))
		{
			echo '<a href=?mod='.$mod.'&id_current='.$row["Business_ID"].'>';
			echo 'Sub Item';
			echo '</a>';
		}
		else
		...
		<td align="center">
		<input name="" type="checkbox" <? if ($row["Business_Istop"]==1){?> checked <? }?> onClick="document.location.href='module/Settop.php?id=<?=$row["Business_ID"];?>&mod=<?=$mod;?>&id_current=<?=$id_current;?>&kind=<?=$kind;?>';"></td>			
    	<td align="center">
		<a href="?mod=<?=$mod?>&id=<?=$row["Business_ID"];?>&act=edit&id_current=<?=$id_current?>&curPg=<?=$curPg?>&lang=<?=$lang?>"><img alt="Sửa" src="skin/icon_edit.gif" width="16" height="16" border="0"></a></td>
    	<td align="center">
		<a href="module/Del.php?mod=<?=$mod?>&id=<?=$row["Business_ID"];?>&id_current=<?=$id_current?>&curPg=<?=$curPg?>&lang=<?=$lang?>" onclick="javascript:return confirm('Bạn có muốn xóa không ?')">
...
<!--------------------------------------------------------------------->
	...
	if ($act=="edit")
	{
		$sqlEdit = "select * from tbl_Business where Business_ID=".$id;
		$result = mysql_query($sqlEdit,$cnn);	
		if ($rowEdit=mysql_fetch_array($result))
		{
			$Business_Name		=$rowEdit['Business_Name'];			
			$Business_Image		=$rowEdit['Business_Image'];
			$Business_Teaser		=$rowEdit['Business_Teaser'];
			$Business_Content	=$rowEdit['Business_Content'];
			$f_action		='Edit.php';
			$btnSubmit		="Edit";
		}
	}
	elseif (($act=="add") || ($act==""))
		{				
			$Business_Name		="title";				
			$Business_Image		="";				
			$Business_Teaser	="qoute";
			$Business_Content	="detail";
			$f_action		='Add.php';
			$btnSubmit		="Add";
		}
	?>
	...
	<tr>
   		<td><input name="Business_Name" type="text" value="<?=$Business_Name;?>" style="width:100%; border:1px solid #CCCCCC "></td>
  	</tr>	
	<tr>
   		<td>Image <input type="text" id="inpImgURL" name="inpImgURL" class="inpTxt" value="<?=$Business_Image?>" style="width:88%; border:1px solid #CCCCCC; height:18px; ">				
	...
	<tr>
    	<td><textarea name="Business_Teaser"><?=$Business_Teaser;?></textarea>		
		<script>
			...
			oEdit1.REPLACE("Business_Teaser");
		</script>	</td>
  	</tr>  	
  	<tr>
    	<td><textarea name="Business_Content"><?=$Business_Content;?></textarea>		
		<script>
			...
			oEdit2.REPLACE("Business_Content");
		</script>	</td>
  	</tr>  
	<tr>
   	 	<td align="center">
